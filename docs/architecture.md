...
| `webapp/` | Flask product UI (ADR-0013): routes + Jinja templates + responsive CSS design system over the unchanged `ui/` core; stateless GET-param searches; plotly.js served from the installed package; calibrated refusal state (ADR-0019), privilege/PII badges (ADR-0020), `/audit` trail (ADR-0022); client-safe citations only (ADR-0018); case-study layer with guided tour, glossary, and `/document/<id>` source view (ADR-0023); mobile-safe evidence workspace and graph controls (Decision 0024) |
