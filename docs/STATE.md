...
Migration verification: Ruff passed; `.venv/bin/python -m pytest -q` passed 174 tests with two embedding-parity skips caused by unavailable model clients. The `uv run pytest` launcher has a stale pre-move shebang pointing at the former `Development/Projects` path; recreate the ignored virtual environment separately rather than treating that generated-path issue as a product failure.
